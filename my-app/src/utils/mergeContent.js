...
const VALID_NAV_IDS = new Set(['home', 'work', 'about', 'process', 'contact']);

function getNavId(href) {
  const raw = (href || '').trim();
...
function normalizeNavLinks(defaults, loaded) {
  const links = loaded?.navLinks?.length ? loaded.navLinks : defaults.navLinks;
  const filtered = links.filter((link) => VALID_NAV_IDS.has(getNavId(link.href)));
  return filtered.length ? filtered : defaults.navLinks;
}
...
export function mergeSiteContent(loaded) {
  ...
  return {
    ...
    hero: {
      ...mergeSection(defaultContent.hero, loaded.hero),
      navLinks: normalizeNavLinks(defaultContent.hero, loaded.hero),
    },
